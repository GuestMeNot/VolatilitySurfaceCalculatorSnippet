package com.nadtech.stockprogram.options;
...
import com.nadtech.stockprogram.statistics.StatisticsUtils;
import com.nadtech.stockprogram.utils.FastDate;
import com.nadtech.stockprogram.utils.SimpleStatisticsUtils;
import com.nadtech.stockprogram.valueobjs.OptionData;
import com.nadtech.stockprogram.valueobjs.TimeSeriesPointGroup;
...
public class SVI {
	...
	private double a, b, p, sigma, sigmasigma, m = Double.NaN;
	...
	public abstract class MinimizerSVI implements MinimisationFunction, MultivariateFunction {
		...
		public MinimizerSVI(OptionData[] data, double[] initParams, boolean ssr) {
			...
			this.calcSSR = ssr;
			currentValue = data[0].getCurrentStockPrice();
		}
		...
		public final double getA(double[] param) { return isASet() ? a : param[getIndex(0)]; }
		public final double getB(double[] param) { return isBSet() ? b : param[getIndex(1)]; }
		public final double getP(double[] param) { return isPSet() ?  p : param[getIndex(2)]; }
		public final double getM(double[] param) { return isMSet() ? m : param[getIndex(3)]; }
		public final double getSigma(double[] param) { return isSigmaSet() ? sigma : param[getIndex(4)]; }
		
		public final boolean isASet() { return !Double.isNaN(a); }
		public final boolean isBSet() { return !Double.isNaN(b); }
		public final boolean isPSet() { return !Double.isNaN(p); }
		public final boolean isMSet() { return !Double.isNaN(m); }
		public final boolean isSigmaSet() { return !Double.isNaN(sigma); }
...
//			if(!isMinIVOk(a, b, p, m, sigma, sigma*sigma, t)) {
//				return Double.MAX_VALUE;
//			}
			...
			// TODO Check for Calendar arbitrage!!!!
...
//			System.out.println("quasi: " + isQuasySVI_OK(a, b, p, m, sigma, t, w));
	...
	public static boolean isQuasySVI_OK(double a, double b, double p, double m, double sigma, double t, double[] w_t) {
		...
		if(absD > c || absD > sigma4 - c) {
			return false;
		}
		
//		double maxVt = NumberUtils.max(w_t);
...
//	public static boolean isMinIVOk(double a, double b, double p, double m, double sigma, double sigmasigma, double t) {
//		
//		double k = getMinIVLogStrike(m, p, sigma);
//		double w_t = calculateSVI(a*t, b*t, p, k, m, sigma, sigmasigma, t);
//		
//		// We don't need to convert from total variance to IV since both should be above zero.
//		if(w_t < 0) {
//			return false;
//		}
//		
//		double iv = getIVFromWT(w_t, t);
//		
//		if(iv > 1) {
//			return false;
//		}
...
//	}
	...
	private void optimizeSVI(MinimizerSVI functSVI, MultivariateOptimizer optimizer, double[] initParams, double[] startParams) {
    	...
    	double[] param = functSVI.minParams; // pointValueCDA.getPoint();
...
//		MultivariateOptimizer powell = new SimplexOptimizer(.00001d, .00001d);
...
//		MinimizerSVIJW functjw = new MinimizerSVIJW(data);
//        
//        // Set the raw parameters so that we can use them to calculate the three constant JW params: v_v, phi_t and p_t
//        functjw.initialA = a;
//        functjw.initialB = b;
//        functjw.initialP = p;
//        functjw.initialM = m;
//        functjw.initialSigma = sigma;
//
//        // get the starting values for optimizing JW.
//        double p_t = functjw.getPT();
//        double phi_t = functjw.getPhiT();
//        double v_t = functjw.getVT();
//        
//        double ctPrime = getCTPrime(p_t, phi_t);
//        double vMinTPrime = getVMinTPrime(p_t, v_t, ctPrime);
//        
//    	PointValuePair jwPointValue = optimizer.optimize(10000, functjw, GoalType.MINIMIZE, new double[] { ctPrime, vMinTPrime });
//
//    	double[] jwParam = jwPointValue.getPoint();
//    	
//    	// recover the optimized JW params;
//    	double c_t = jwParam[0];
//    	double vtMin = jwParam[1];
//    	
//    	double t = getT();
//    	double w_t = getW_t(v_t, t);
//    	this.b = getB_JW(w_t, c_t, p_t);
//    	this.p = getP_JW(w_t, this.b, p_t);
//
//		double alpha = getAlpha(this.p, phi_t, w_t, this.b);
//
//		this.m = getM_JW(v_t, vtMin, t, this.b, this.p, alpha);
//		this.sigma = getSigma_JW(alpha, this.m);
//		this.sigmasigma = sigma*sigma;
//		this.a = getA_JW(vtMin, this.b, this.p, this.sigma, t);
    			
	}

	public static TimeSeriesPointGroup[] fillInOptionData(OptionData[] data) {
		
		List<Double> dtes = OptionData.getDtes(data);
		List<OptionData> list = new ArrayList<OptionData>();

		BoolHolder sviCalc = new BoolHolder();
		for(double dte : dtes) {
			list.addAll(fillInOptionData(data, dte, true, sviCalc));
			list.addAll(fillInOptionData(data, dte, false, sviCalc));
		}

		TimeSeriesPointGroup[] newData = list.toArray(new OptionData[list.size()]);
	...
	public static List<OptionData> fillInOptionData(OptionData[] dataAll, double dte, boolean calls, BoolHolder sviCalc) {

		OptionData[] dteData = OptionData.getOptionsByDte(dataAll, dte);
		
		OptionData[] callPutData = OptionData.getCallOptionData(dteData);
		if (!calls) {
			callPutData = OptionData.getPutOptionData(dteData);
		}
		
		OptionData[] data = OptionData.getOTMOptions(callPutData);
//		OptionData[] ivData = OptionData.getOptionsWithIVBetween(otmData, .01, .90);
//		OptionData[] data = OptionData.getOptionsWithDeltaAbove(ivData, .03d);
		
		if(data.length == 0) {
			
			String dateStr = "";
			if(dataAll.length > 0) {
				dateStr += dataAll[0].getDate() + ":";
			}
			
			// Sometimes there aren't quotes for longer dated options!
			if(dte < 115) {
				System.err.println("WARNING: " + dateStr + " no data available for dte: " + dte);
			}
		...
		OptionData ref = data[0];
		double currentValue = ref.getCurrentStockPrice();
		
		List<OptionData> list = new ArrayList<OptionData>();
		List<Double> strikeList = new ArrayList<Double>();
		
		double[] moduli = OptionData.getStrikeModuli(data);
		
		OptionData.OptionDataMap strikeMap = OptionData.getOptionDataMap(data, OptionData.STRIKE_PRICE, 2); 
		
		for (double modulus : moduli) {
			
			double lowestStrike = OptionData.getModulusLowestStrike(data, modulus);
			double highestStrike = OptionData.getModulusHighestStrike(data, modulus);
			
			if(lowestStrike > highestStrike) {
				
				double temp = lowestStrike;
				lowestStrike = highestStrike;
				highestStrike = temp;
						
			}
			
			
			for (int i = 0; lowestStrike + (i*modulus) < highestStrike; i++) {

				
				double currentStrike = lowestStrike + (i*modulus);
				if(strikeList.contains(currentStrike)) {
					continue;
				}
				
				OptionData strikeData = strikeMap.get(currentStrike);
				if (strikeData == null) {

					sviCalc.bool = true;

					OptionData[] closestData = OptionData.getClosestStrikes(data, currentStrike, 2);
					strikeData = createOptionDataFromSviParams(closestData, currentStrike);
					if(strikeData == null) {
						continue;
					}

					if(!strikeData.contains(OptionData.SVI_A)) {
						throw new IllegalStateException();
					}

					strikeList.add(currentStrike);
	...
	public static double calculateIV(OptionData[] closestData, double currentStrike) {
		...
		double sum = StatisticsUtils.sum(ivs);
...
//			System.err.println("BAD IV : " + date + " dte: " + dte + " currentStrike: " + currentStrike + " IV: " + impliedVolatility);
		...
		if(optionPrice < .01d) {
//			System.err.println("SVI date: " + data[0].getDate() + "  strike: " + currentStrike + " optionPrice: " + optionPrice + "  BS(" + calls + "," + currentValue + "," + currentStrike + "," + T + "," + r + "," + impliedVolatility + ")");
			double iv = data[0].getIV();
			double strikeToCheck = data[0].getStrikePrice();
			double mid = data[0].getMid();
			double impliedVolatility0 = calculateIV(data, strikeToCheck);			
			if(impliedVolatility0 > 1d) {
				impliedVolatility0 = 1d;
			}
			double optionPrice0 = BlackScholes.optionPrice(calls, currentValue, strikeToCheck, dte, r, impliedVolatility0);
			if(iv > 1d) {
				iv = 1d;
			}
			double optionPrice1 = BlackScholes.optionPrice(calls, currentValue, strikeToCheck, dte, r, iv);
//			System.err.println("SVI date: " + data[0].getDate() + " dte: " + data[0].getValue(OptionData.DAYS_TO_EXPIRATION) + "  strike: " + strikeToCheck + " optionPrice: (^" + optionPrice0 + " ~= " + mid + " ~= " + optionPrice1 + ")   BS(" + calls + "," + currentValue + "," + strikeToCheck + "," + T + "," + r + ",(^" + impliedVolatility0 + " ~= " + iv + ")");
			
		}
		
		OptionData strikeData = OptionData.createOptionData(optionPrice, data[0].getStockTicker(), calls, date, data[0].getExpiryDate(), 
	...
	public static double calculateCurrentStrikeForExactDelta(OptionData[] data, double delta) {
		...
		double val = strike1 + (percentage*totalStrikeDistance);
		return val;

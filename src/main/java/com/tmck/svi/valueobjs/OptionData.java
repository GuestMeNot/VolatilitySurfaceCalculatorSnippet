...
import com.tmck.svi.utils.StatisticsUtils;
import com.tmck.svi.utils.StringUtils;

/**
 * @author tim
 */
public class OptionData extends MultiValueTimeSeriesPoint implements SymbolHolder, Cloneable {
	...
	/**	 Gram-Chevalier: Marginal effect of nonnormal Kurtosis for a normalized IV. */
	public static final String Q4 = "Q4";

	/**	 Gram-Chevalier: Marginal effect of nonnormal Skewness for a normalized IV. */
	public static final String Q3 = "Q3";
	
	/** Gram-Chevalier: Implied Stddev with skew and kurtosis. */
	public static final String ISD = "ISD";
	
	/** Gram-Chevalier: Implied Skew. */
	public static final String ISK = "ISK";
	
	/** Gram-Chevalier : Implied Kurtosis. */
	public static final String IKT = "IKT";

	/** Generalized Beta P. */
	public static final String GB_P = "GB_P";

	/** Generalized Beta Q. */
	public static final String GB_Q = "GB_Q";
	
	/** Generalized Beta A. */
	public static final String GB_A = "GB_A";
	...
	private Symbol volCalcSymbol = null;
    ...
    public final Symbol getStockTicker() { return stockTicker; }
    public final void setOptionTicker(Symbol optionTicker) { this.optionTicker = optionTicker; }
    public final void setStockTicker(Symbol stockTicker) { this.stockTicker = stockTicker; }

    public Symbol getSymbol() { return getOptionTicker(); }
...
//    	dte = -1;
//    	currentStockPrice = Double.NaN;
//    	strikePrice = Double.NaN;
    	iv = Double.NaN;
    	
    }
    
	public static List<OptionData> getCallsGreaterThanDelta(OptionData [] data, double delta) {
		List<OptionData> calls = getCalls(data);
		ListIterator<OptionData> itr = calls.listIterator();
		while(itr.hasNext()) {
			OptionData optiondata = itr.next();
			double optionDelta = optiondata.get(OptionData.DELTA_PER_OPTION).getValue();
			if(optionDelta < delta) {
				itr.remove();
			}
		}
		return calls;
	...
	public static List<OptionData> getPutsGreaterThanDelta(OptionData [] data, double delta) {
		List<OptionData> puts = getPuts(data);
		ListIterator<OptionData> itr = puts.listIterator();
		while(itr.hasNext()) {
			OptionData optiondata = itr.next();
			double optionDelta = optiondata.get(OptionData.DELTA_PER_OPTION).getValue();
			if(Math.abs(optionDelta) < Math.abs(delta)) {
				itr.remove();
			}
		}
		return puts;
...
//			if(currentDistance == distance && !OptionData.equalRoots(atm, data[i])) {
//				System.err.println("Error: duplicate options found: atm-root: " + atm.getRoot() + " current-root: " + data[i].getRoot());
//			}

			String foundRoot = atm.getRoot();
			if(foundRoot != null && !foundRoot.startsWith(desiredRoot)) {
//				System.err.println("Error: unexpected options root found: " + foundRoot + "   expected the root to start with: " + desiredRoot);
				continue;
			}
	...
	public static OptionData getNextOptionStrike(OptionData [] data, OptionData current, boolean otm) {
		double currentStrike = current.getStrikePrice();
		boolean call = current.isCall();
		return getNextOptionStrike(data, currentStrike, call, otm);
	}
	...
	/** @see http://www.sheridanmentoring.com/sigma */
	public double getBlackScholesStandardDeviation(int calendarDays) {
		
		double iv = getIV();
		double price = getCurrentStockPrice();
		
		double t_sqrt = Math.sqrt(((double) calendarDays) / 365.25d);
		
		return price * iv * t_sqrt;
		
	}
	
	public static OptionData getAtmOption(OptionData[] data, boolean calls) {

		if(data == null) {
			return null;
		}

		List<OptionData> newData = null;
		if(calls) {
			newData = getCalls(data);
		} else {
			newData = getPuts(data);
		}
		return getAtmOption(newData.toArray(new OptionData[newData.size()]));
	}	

	public static OptionData getAtmOptionForStockPrice(OptionData[] data, double stockPrice, boolean calls) {
		...
		return getAtmOptionForStockPrice(newData.toArray(new OptionData[newData.size()]), stockPrice);
	}

	/** 
	 * Find the option leg near a given delta.
	 * This signature is most often called for modeling an IV surface.
	 */
	public static OptionData getOptionNearDelta(OptionData[] data, double delta) {
		return getOptionNearDelta(data, delta, Double.NaN);
	}
	
	public static boolean isBidAskValid(OptionData[] data, double bidThreshold, int numExpectedBids) {
		int bids = getNumBids(data, bidThreshold);
		return bids >= numExpectedBids;
	}
	
	public static int getNumBids(OptionData[] data, double bidThreshold) {
		
		int count = 0;
		double[] bids = getValues(data, OptionData.BID);
		for(double bid : bids) {
			if(bid > bidThreshold) {
				count++;
			}
	...
	public static boolean isDeltasAppearCorrect(OptionData[] data, double deltaThreshold) {

		double max = getMaxValue(data, OptionData.DELTA_PER_OPTION);
		return max > deltaThreshold;
		
	}
	
	public static double getMaxValue(OptionData[] data, String name) {
		double[] values = getValues(data, name);
		return SimpleStatisticsUtils.getMax(values);
	}
	 ...
	 * This signature is most often called for modeling an IV surface.
	 * 
	 * @param strikeModulus - if you want only options that are evenly divisible by 10 dollars then pass in 10.
	 */
	public static OptionData getOptionNearDelta(OptionData[] data, double delta, double strikeModulus) {

		if(data == null) {
			return null;
		}

		OptionData closestOption = null;
		double maxDistance = Double.MAX_VALUE;
		
		// Delta is negative for puts so we take the absolute value. 
		double absDelta = Math.abs(delta);
		
		for(int i = 0; i < data.length; i++) {
			
			double strike = data[i].getStrikePrice();
			if(!isStrikeModulus(strike, strikeModulus)) {
				continue;
			}
			
			// Delta is negative for puts so we take the absolute value. 
			double currentDelta = Math.abs(data[i].get(OptionData.DELTA_PER_OPTION).getValue());
			
			
			double distance = Math.abs(absDelta - currentDelta);
			if(distance < maxDistance) {
				closestOption = data[i];
	...
	public static boolean isStrikeModulus(double strike, double strikeModulus) {
		
		// If we don't have a strike modulus then we default to a match.
		if(Double.isNaN(strikeModulus)) {
			return true;
		}
		
		// get the modulus (remainder after division)
		double remainder = Math.abs(strike%strikeModulus);
		
		// If the remainder is less than 1 penny the it is the same strike.
		return remainder < .01d;
...
//				bid = roundIfWithinTolerance(bid);
	...
	public static void removeLowOpenInterestOptions(List<OptionData> optionDataList, double openInterest) {
		...
		while(itr.hasNext()) {
			...
			// If we interpolated the data there there won't be open interest.
			...
			if(Math.abs(oi) < openInterest) {
				...
				strikes += " " + strike + (call ? "C" : "P");
				itr.remove();
			}
		}
		
		if(optionDataList.size() == 0) {
			System.err.println("Oops! removed all OptionData due to low Open Interest: " + symbol + " : " + strikes);
		}
	...
	public static void removeOptionsBelowDelta(List<OptionData> optionDataList, double optionDelta) {
		...
		if(optionDataList.size() == 0) {
			System.err.println("Oops! removed all OptionData due to low Delta: " + symbol + " : " + strikes);
		}		
	}
	...
	public static void removeOptionsAboveDelta(List<OptionData> optionDataList, double optionDelta) {
		...
		if(optionDataList.size() == 0) {
			System.err.println("Oops! removed all OptionData due to high Delta: " + symbol + " : " + strikes);
		}
	...
	public static OptionData[] removeOptionsWithDteStrictlyLessThan(OptionData[] data, double maxDte) {
		...
		return list.toArray(new OptionData[list.size()]);
	...
	public OptionData copy() {
		...
		copy.setCall(isCall());
		copy.setCombinationStategy(getCombinationStategy());
	...
	public OptionData convertToUnderlying(double multiplier) {
		OptionData data = copy();
		data.setUnderlying(multiplier);
		return data;
	}

	/** add strikes from data1 that are in data2. */
	public static OptionData[] intersection(OptionData[] data1, OptionData[] data2) {
		
		List<OptionData> list = new ArrayList<OptionData>();
		for(OptionData data : data1) {
			for(OptionData data_2 : data2) {
				double data_strike = data.getStrikePrice();
				double data_2_strike = data_2.getStrikePrice();
				if(data_strike == data_2_strike) {
					list.add(data);
				}
			}
		}
	 ...
	 */
	public static OptionData[] removeOptions(OptionData[] data1, OptionData[] optionsToRemove) {

		List<OptionData> list = new ArrayList<OptionData>();
		for(OptionData data : data1) {
			boolean add = true;
			for(OptionData data_2 : optionsToRemove) {
				double data_strike = data.getStrikePrice();
				double data_2_strike = data_2.getStrikePrice();
				if(data_strike == data_2_strike) {
					add = false;
	...
	public static OptionData[] removeWeeklies(OptionData[] data) {
		return removeWeeklies(data, 0);
	}
	
	public static OptionData[] removeWeeklies(OptionData[] data, int daysForward) {
		if(data == null || data.length == 0) {
			return data;
		}
		FastDate expiryDate = ExpirationFinder.findCurrentMonthlyExpiration(daysForward);
		return keepOptionsExpiringOnDate(data, expiryDate);
	}
	
	public static OptionData[]  getOptionsExpiringOnDate(OptionData[] data, FastDate date) {
		return keepOptionsExpiringOnDate(data, date);
	}
	...
	public double getOtmAmount() {
    	...
    	return Math.abs(currentPrice - strikePrice);
	...
	public static double getExpectedStddev(OptionData[] data) {
		
		OptionData call = getAtmOption(data, true);
		OptionData put = getAtmOption(data, false);

		String str = "because data length is 0";
		if(data != null && data.length > 0 && (call == null || put == null)) {
			str = " data length is " + data.length + " date: " + data[0].getDate();
		}
		
		double callMid = call != null ? call.getMid() : Double.NaN;
		double putMid = put != null ? put.getMid() : Double.NaN;
		
		if(call == null) {
			getAtmOption(data, true); // For Debugging
			System.err.println("ExpectedStddev: No ATM call: " + str);
			if(put != null) {
				return putMid*2d;
			}
		}
		...
		if(put == null) {
			getAtmOption(data, false); // For Debugging
			System.err.println("ExpectedStddev: No ATM put: " + str);
			if(call != null) {
				return callMid*2d;
			}
		}
	...
	public static double getSkewSlope(OptionData[] data) {
		
		OptionData[] calls = getCallOptionData(data);
		OptionData callAt25Delta = OptionData.getOptionNearDelta(calls, .25d);

		OptionData[] puts = getPutOptionData(data);
		OptionData putAt25Delta = OptionData.getOptionNearDelta(puts, -.25d);

		if(callAt25Delta == null) {
			// NOTE: put call parity
			callAt25Delta = OptionData.getOptionNearDelta(puts, -.75d);
		}

		if(putAt25Delta == null) {
			// NOTE: put call parity
			putAt25Delta = OptionData.getOptionNearDelta(calls, .75d);
		}

		double strikeDistance = getStrikeDistance(putAt25Delta, callAt25Delta);
		double ivDistance = getIVDistance(putAt25Delta, callAt25Delta);
		
		return ivDistance/strikeDistance;
	...
	public static double getStrikeDistance(OptionData put, OptionData call) {

		if(put == null || call == null) {
			return Double.NaN;
		}

		double callStrike = call.getStrikePrice();
		double putStrike = put.getStrikePrice();

		// TODO this doesn't seem correct the upper value is subtracted from the lower value for the slope!!!!!
		double callPutStrikeDistance = callStrike - putStrike;
	
		return callPutStrikeDistance;
	}
		
	public static OptionData[] getCallOptionData(OptionData[] data) {
		
		List<OptionData> callList = OptionData.getCalls(data);
		return callList.toArray(new OptionData[callList.size()]);
	...
	public static OptionData[] getPutOptionData(OptionData[] data) {
		...
		return putList.toArray(new OptionData[putList.size()]);
	...
	public static OptionData[] getOtmCallsForDte(OptionData[] data, double dte) {
	
		OptionData[] dteData = getOptionsByDte(data, dte);
		OptionData[] otmcalls = getOTMOptions(dteData);
		List<OptionData> calls = getCalls(otmcalls);
		return calls.toArray(new OptionData[calls.size()]);
		
	}

	public static OptionData[] getOtmPutsForDte(OptionData[] data, double dte) {
		
		OptionData[] dteData = getOptionsByDte(data, dte);
		OptionData[] otmcalls = getOTMOptions(dteData);
		List<OptionData> calls = getPuts(otmcalls);
		return calls.toArray(new OptionData[calls.size()]);
	...
	public static boolean isWeeklyRoot(String root) {
		return root.endsWith("W");
	}

	public static boolean isQuarterlyRoot(String root) {
		return root.endsWith("Q");
	}

	public static boolean equalRoots(OptionData data1, OptionData data2) {
		String root2 = data2.getRoot();
		return equalRoots(data1, root2);
	}
	
	public static boolean equalRoots(OptionData data1, String root) {
		String root1 = data1.getRoot();
		String root2 = root;
		return equalRoots(root1, root2);
	}

	public static boolean equalRoots(String root1, String root2) {
	...
	public static OptionData[] removeOption(OptionData[] data, OptionData toRemove) {
		return removeOptions(data, new OptionData[] { toRemove });
	}

	public static double[] getStrikeModuli(OptionData[] data) {
		
		double[] strikes = OptionData.getStrikePrices(data);
		
		double[] potentialStrikeModuli = { 25d, 10d, 5d, 2.5d, 1d, .50, .25 };
		double currentValue = data[0].getCurrentStockPrice();
	
		int hiIdx = getHighestPotentialStrikeModulusIdx(strikes, potentialStrikeModuli, currentValue);
		int lowIdx = getLowestPotentialStrikeModulusIdx(strikes, potentialStrikeModuli, currentValue, hiIdx);

		double[] strikeModuli = Arrays.copyOfRange(potentialStrikeModuli, hiIdx, lowIdx);
		return strikeModuli;
	}

	private static int getHighestPotentialStrikeModulusIdx(
			double[] strikes, double[] potentialStrikeModuli, double currentValue) {
		for(int i = 0; i < potentialStrikeModuli.length; i++) {
			double moduli = potentialStrikeModuli[i];
			double modulus = currentValue / moduli;
			if(modulus >= 1) {
				if(containsModulus(strikes, moduli)) {
					return i;
				}
			}
		}
	...
	/** Find the smallest strike modulus.
	 * So we start just below the mid point and continue towards the ATM until we can't find the strike.
	 * @param data 
	 */
	private static int getLowestPotentialStrikeModulusIdx(
			double[] strikes, double[] potentialStrikeModuli, double currentValue, int hiIdx) {

		if(hiIdx == potentialStrikeModuli.length) {
			return -1;
		}

		for(int i = hiIdx + 1; i < potentialStrikeModuli.length; i++) {
			if(!containsModulus(strikes, potentialStrikeModuli[i])) {
				return i;
			}
		}
		
		return potentialStrikeModuli.length;
				
	}

	private static boolean containsModulus(double[] strikes, double modulus) {
		
		for(double strike : strikes) {
			
			// if the modulus leaves a remainder that is odd (not divisible by 2* the modulus) 
			// then we can assume the modulus is in the data.
			double mod2 = strike % (modulus*2);
			
			// Now we make the modulus odd.
			double mod = mod2 - modulus;
	...
	/** Find the lowest strike for the modulus. */
	public static double getModulusLowestStrike(OptionData[] data, double modulus) {
		
		sortByStrikeAscending(data);
		
		for(OptionData item : data) {
			double strike = item.getStrikePrice();
			if(isStrikeAtModulus(item, modulus)) {
				double targetStrike = strike + modulus;
				OptionData nextHigherStrike = getOptionDataByStrike(data, targetStrike);
				
				if(nextHigherStrike != null) {
					return strike;
				}
			}
		}
	...
	public static boolean isStrikeAtModulus(OptionData item, double modulus) {
		
		double strike = item.getStrikePrice();
		double remainder = strike % modulus;
	...
	public static class StrikeComparator implements Comparator<OptionData> {

		@Override
		public int compare(OptionData arg0, OptionData arg1) {
			double strike1 = arg0.getStrikePrice();
			double strike2 = arg1.getStrikePrice();
			if(areStrikesEqual(strike1, strike2)) {
				return 0;
			}
			if(strike1 < strike2) {
				return -1;
	...
	private static final StrikeComparator strikeComparator = new StrikeComparator();
	...
	public static OptionData[]  convertToArray(List<OptionData> list) {
		...
		OptionData[] arr = new OptionData[list.size()];
	...
	public static OptionData[] sortByStrikeAscending(OptionData[] data) {
		
		List<OptionData> list = convertToList(data);
		Collections.sort(list, strikeComparator);
		return convertToArray(list);
		
	}

	public static double getModulusHighestStrike(OptionData[] data, double modulus) {
		
		sortByStrikeAscending(data);
		
		double lastStrike = Double.NaN;
...
//		String value = String.format("%05f", strike);
	...
	/** @see A note on sufficient conditions for no arbitrage by Peter Carr a, Dilip B. Madan */
	public static void isArbitragePresent(OptionData[] data) {
		isArbitragePresent(data, null);
	}
	
	/** @see A note on sufficient conditions for no arbitrage by Peter Carr a, Dilip B. Madan */
	public static void isArbitragePresent(OptionData[] data, Boolean calc) {

		String str = (calc == null || calc == false) ? "" : " Surface-Calculated: " + calc;
	...
	public static boolean hasButterflyArbitrage(OptionData[] data, boolean calls) {
		...
		for(int jj = 0; jj < dtes.size(); jj++) {
			
			OptionData[] dteData = getOptionsByDte(data, dtes.get(jj));
	...
	public static boolean hasCalendarArbitrage(OptionData[] data, boolean calls) {
		...
		for(int i = 0; i < dtes.size(); i++) {
			...
			for(int j = i+1; j < dtes.size(); j++) {
				...
				for(int k = 0; k < dte1Data.length; k++) {
					
					OptionData data1 = dte1Data[k];
	...
	public static boolean hasVerticalArbitrage(OptionData[] data, boolean calls) {
		...
		for(int j = 0; j < dtes.size(); j++) {
			
			OptionData[] dteData = getOptionsByDte(data, dtes.get(j));
			...
			for(int i_1 = 0; i_1 < dteData.length; i_1++) {
				...
				for(int i = i_1+1; i < dteData.length; i++) {
					...
					// We we make free money (NOTE: the value is normalized to the range [0, 1] )
					
					// An ITM call should be worth more than an OTM call and credit should be less than strike width.
					// i.e. the numberator must be positive and the ratio must be less than 1
	...
	public static double getLogStrike(double strike, double currentValue) {
		double logStrike = Math.log(strike/currentValue);
		return logStrike;
	}
	...
	public static boolean isImpliedVolatilityValid(OptionData data) {
		...
		if(iv < 0 || iv > 1 || Double.isInfinite(iv) || Double.isNaN(iv)) {
			return false;
		}
	...
	public static OptionData[] getOptionsWithIVBetween(OptionData[] data, double lowIV, double highIV) {
		
		List<OptionData> list = new ArrayList<OptionData>();
		for(OptionData item : data) {
			if(isIVBetween(item, lowIV, highIV)) {
				list.add(item);
			}
		}
	...
	public static boolean isIVBetween(OptionData data, double lowIV, double highIV) {
		double iv = data.getIV();
		return iv > lowIV && iv < highIV;
	}

	public static OptionData[] getOptionsWithDeltaAbove(OptionData[] data, double delta) {
		List<OptionData> list = new ArrayList<OptionData>();
		for(OptionData item : data) {
			if(isDeltaAbove(item, delta)) {
				list.add(item);
			}
		}
	...
	public static boolean isDeltaAbove(OptionData data, double delta) {
		double d = data.getValue(OptionData.DELTA_PER_OPTION);
	...
	/** Get the closest strikes for either calls or puts. This method doesn't work for a mixture of calls and puts. */
	public static OptionData[] getClosestStrikes(OptionData[] data, double strike, int numStrikesAboveAndBelow) {
		
		sortByStrikeAscending(data);

		int idx = 0;
		double[] strikes = getStrikePrices(data);
		for(int i = 0; i < data.length; i++) {
			double currentStrike = strikes[i];
			if(currentStrike > strike) {
				break;
			}
			idx = i;
		}
		return getOptionDataAboveAndBelowIndex(data, numStrikesAboveAndBelow, idx);
	}

	/** Get the closest deltas for either calls or puts. This method doesn't work for a mixture of calls and puts. */
	public static OptionData[] getClosestDeltas(OptionData[] data, double delta, int numStrikesAboveAndBelow) {
		...
		int idx = 0;
		double[] deltas = OptionData.getDeltas(data);
		for(int i = 0; i < data.length; i++) {
			double currentDelta = Math.abs(deltas[i]);
			if(currentDelta > absDelta) {
				break;
			}
			idx = i;
		}
	...
	public static OptionData[] getOptionDataAboveAndBelowIndex(OptionData[] data, int numStrikesAboveAndBelow, int idx) {
		
		if(idx < numStrikesAboveAndBelow) {
			idx = 0;
		}
		
		int numStrikes = numStrikesAboveAndBelow*2;
		int max = data.length - numStrikes;
		if(idx > max) {
			if(max < 0) {
				idx = 0;
			} else {
		...
		// If we are need to backup more than is there is available data for then we set the idx to 0
		if(numStrikes > data.length || idx+1 < numStrikesAboveAndBelow) {
			idx = 0;
			// If we are looking for more data than is available we use all the data that is available.
			if(numStrikes > data.length) {
				numStrikes = data.length;
			}
		} else {
			idx -= (numStrikesAboveAndBelow - 1); 
		}
		
		OptionData[] newData = new OptionData[numStrikes];
	...
	public static double[] getMidPrices(OptionData[] data) {
		double[] values = new double[data.length];
		for(int i = 0; i < data.length; i++) {
			values[i] = data[i].getMid();
		}
	...
	public static void calculateValuesFromIVs(List<OptionData> data) {
		
		for(OptionData datum : data) {
			BlackScholes.calculateValuesFromIV(datum);
		}
		
	}
	
	public static double getRiskReversalIV(OptionData[] data) {
		
		OptionData[] calls = getCallOptionData(data);
		OptionData[] puts = getPutOptionData(data);
		
		OptionData call = getOptionNearDelta(calls, .25d);
		OptionData put = getOptionNearDelta(puts, .25d);
		
		return OptionData.getIVDistance(put, call);
		
	}

	public static double getFlyIV(OptionData[] data) {

		OptionData[] calls = getCallOptionData(data);
		OptionData[] puts = getPutOptionData(data);

		OptionData atmData = OptionData.getOptionNearDelta(puts, .5d);
		OptionData call = getOptionNearDelta(calls, .25d);
		OptionData put = getOptionNearDelta(puts, .75d);
		
		return OptionData.getIVDistance(put, atmData) +
				OptionData.getIVDistance(call, atmData);
		
	}

	public static boolean isSameDte(OptionData[] data) {
		
		List<Double> dtes = getDtes(data);
		return dtes.size() == 1;
		
	}

	public static boolean isFarAwayStrike(double strike, double stockPrice, double percentItm) {
		double percent = (strike - stockPrice)/stockPrice; 
		return Math.abs(percent) > percentItm;
	}
	...
	public boolean isVolatilityArbitrageSell() {
		double volArbOffset = getVolArbOffset();
		return isVolatilityArbitrageSell(volArbOffset);
	}

	public boolean isVolatilityArbitrageSell(double volArbOffset) {
		double iv = getIV();
		double originalIV = getOriginalIV();
		return originalIV > (iv+volArbOffset);
	}

	public boolean isVolatilityArbitrageByPriceSell(double priceOffset) {
		double mid = getMid();
		double originalPrice = getOriginalMid();
		return mid < (originalPrice+priceOffset);
	}
	
	public boolean isVolatilityArbitrageBuy() {
		double volArbOffset = getVolArbOffset();
		return isVolatilityArbitrageBuy(volArbOffset);
	}

	public boolean isVolatilityArbitrageBuy(double volArbOffset) {
		double iv = getIV();
		double originalIV = getOriginalIV();
		return iv > (originalIV+volArbOffset);
	}

	public boolean isVolatilityArbitrageByPriceBuy(double priceOffset) {
		double mid = getMid();
		double originalPrice = getOriginalMid();
		return mid > (originalPrice+priceOffset);
	}
	...
	public static OptionData getAverageOptionData(OptionData[] data) {
		
		if(data.length == 1) {
			return data[0];
		}

		if(!isSameStockTicker(data)) {
			throw new IllegalStateException("Not the same ticker");
		}
		if(!isSameExpiry(data)) {
			throw new IllegalStateException("Not the same expiry");
		}
		if(!isSameCall(data)) {
			throw new IllegalStateException("Not the same Call/Put");
		}
		if(!isSameRoot(data)) {
			throw new IllegalStateException("Not the same root");
		}
		if(!isSameStrike(data)) {
			throw new IllegalStateException("Not the same root");
		}

		double avgExecutionPrice = getAverageMidPrice(data);
		double avgStockPrice = getAverageStockPrice(data);
		double avgDTE = getAverageDTE(data);

		// This may not return a date the occurs within the hours that the markets are open... but that should be OK.
		long avgDTEMilliseconds = DateUtils.convertDaysToMilliseconds(avgDTE);
		Date avgExecutionDate = DateUtils.addToCalendar(data[0].getExpiryDate(), -avgDTEMilliseconds);

		return createOptionData(avgExecutionPrice, data[0].getStockTicker(), data[0].isCall(), new FastDate(avgExecutionDate), data[0].getExpiryDate(), 
				Double.NaN, avgStockPrice, avgDTE, data[0].getStrikePrice(), data[0].getRoot(), null, data[0].getInterestRate());
		
	}

	public static boolean isSameCall(OptionData[] data) {
		if(data.length < 2) {
			return true;
		}
	...
	public static boolean isSameExpiry(OptionData[] data) {
		if(data.length < 2) {
			return true;
		}
		...
		for(OptionData datum : data) {
			...
			if(!DateUtils.isMatchingDates(datum.getExpiryDate(), data[0].getExpiryDate())) {
				return false;
			}
		}
	...
	public static boolean isSameRoot(OptionData[] data) {
		if(data.length < 2) {
			return true;
		}
		...
		for(OptionData datum : data) {
			...
			if(!(datum.getRoot().equals(data[0].getRoot()))) {
				return false;
			}
		}
	...
	public static boolean isSameStrike(OptionData[] data) {
		if(data.length < 2) {
			return true;
		}
		...
		for(OptionData datum : data) {
			...
			if(!(Math.abs(datum.getDte() - data[0].getStrikePrice()) < .0001d)) {
				return false;
			}
		}
	...
	public static boolean isSameStockTicker(OptionData[] data) {
		if(data.length < 2) {
			return true;
		}
		...
		for(OptionData datum : data) {
			...
			if(!datum.getStockTicker().equals(data[0].getStockTicker())) {
				return false;
			}
		}
	...
	public static double getAverageDTE(OptionData[] data) {
		List<Double> prices = OptionData.getDtes(data);
		double[] d = NumberUtils.convertToArray(prices);
		return SimpleStatisticsUtils.getMean(d);
	}

	public static double getAverageMidPrice(OptionData[] data) {
		double[] prices = OptionData.getMidPrices(data);
		return SimpleStatisticsUtils.getMean(prices);
	}

	public static double getAverageStockPrice(OptionData[] data) {
		double[] stockPrices = OptionData.getStockPrices(data);
		return SimpleStatisticsUtils.getMean(stockPrices);
	}

	public static double[] getStockPrices(OptionData[] data) {
		double[] values = new double[data.length];
		for(int i = 0; i < data.length; i++) {
			values[i] = data[i].getCurrentStockPrice();
		}
		
		return values;	
	}

	public static Date[] getExpiryDates(OptionData[] data) {
		Date[] values = new FastDate[data.length];
		for(int i = 0; i < data.length; i++) {
			values[i] = data[i].getExpiryDate();
		}
...
//		String opraCode = OptionData.createOpraCode(data);
//		data.setOptionTicker(new Symbol(opraCode, Symbol.TICKER));
	...
	/** Synchronized is faster than recreating the DateFormat... profiling using VisualVM */
	@SuppressWarnings("deprecation")
	private static synchronized String formatYYYYMMDD(Date d) {
	...
	public Symbol getVolCalcSymbol() {
		if(volCalcSymbol == null) {
			String root = getRoot();
			Date d = expiryDate;
			String dateStr = formatYYYYMMDD(d);
			volCalcSymbol = new Symbol(root + "-" + dateStr, Symbol.DERIVED_DATA);
		}
		return volCalcSymbol;
	}

	public static double getCorrelationOfChangeInValuesToBidAsk(List<OptionData> data, String name) {
		...
		double[] values1 = new double[deltaSize]; 
		double[] values2 = new double[deltaSize]; 
		for(int i = 0; i < deltaSize; i++) {
			OptionData data1 = data.get(i);
			OptionData data2 = data.get(i+1);
			values1[i] = getChangeInValue(name, data1, data2);
			values2[i] = data2.getBidAskSpread() - data1.getBidAskSpread();
		}
		
		return StatisticsUtils.correlate(values1, values2);
	...
	public static void sortByDateMostRecentFirst(List<OptionData> data) {
			data.sort(new Comparator<OptionData>() {
				@Override
